    ...
    p.draw = function() {
      ...
      for (let x = startX + stepSizeX/2; x <= endX; x += stepSizeX) {
        for (let y = startY + stepSizeY/2; y <= endY; y += stepSizeY) {
          ...
          p.stroke("#D6CFB4"); // Ensure stroke is black and visible
          p.strokeWeight(1);
          ...
          p.noStroke();
          p.fill("#D6CFB4"); // Ensure the circle is white
          // p.circle(x1, y1, 4);
        }
      }
      ...
      for (let i = 0; i < particles.length; i++) {
        ...
        particleLayer.fill("#8D0B41");
        particleLayer.circle(particle.pos.x, particle.pos.y, 2);
      }
      particleLayer.pop();
